...
class Evaluator():
    def __init__(self, eval_params, rel_thresh, rel_func):
        ...
        for metric in EvaluationMetric.__subclasses__():
            ...
            # if m_name == "SER":  # 如果是 SER，传三个参数
            #     new_metric = metric(eval_params[m_name], rel_func, rel_thresh)
            # else:  # 其他 metric 只传 params
            #     new_metric = metric(eval_params[m_name])
...
class RMSE(EvaluationMetric):
    m_names = ["RMSE"]
    def __init__(self,params,rel_func,rel_thresh):# rel_func 和 rel_thresh 仅为接口兼容性而传入，未被使用
        pass
...
# 新加的SER评估
class SER(EvaluationMetric):
    m_names = ["SER"]

    def __init__(self, params, rel_func, rel_thresh):
        self.relevance_func = rel_func
        self.rel_thresh = rel_thresh

    def evaluate(self, actual, pred):
        relevance_scores = self.relevance_func(actual, combine='none')  # 正常使用
        ser_scores = {}
        percentiles = [1, 2, 5, 10, 25, 50, 75]
        for p in percentiles:
            threshold = np.percentile(relevance_scores, 100 - p)
            mask = relevance_scores >= threshold
            ser_rmse = np.sqrt(mean_squared_error(actual[mask], pred[mask], multioutput='raw_values'))
            ser_scores[f"SER{p}%"] = ser_rmse
        return ser_scores
