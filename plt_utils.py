...
strat_cols = {
    ...
    'SMOTER_bin_t' : '#00D6E3', #(Purple)
    'SMOTER_bin' : '#A200FF', #(Cyan)
...
def PlotStepBarResults(results, metric, agg, t, dname, nsteps, forecaster):
    results['Res'] = results['Res'].replace({'GAN_CNN_e0': '1D-Conv GAN','GAN_FNN_e0': '1D-GAN'})
    ...
    lgd = ax.legend(fontsize=10, loc=("upper left"), shadow=True)
    if SAVE_FIGS:
